...
def cake_batch_kl_return_lora_weights(original_model, tokenizer, items_list, target_modules, hparams, test_generation=False):
    ## original ##
...
def cake_batch_kl_sequential_edit(model, tokenizer, items_list, hparams, edit_freq, datatype,test_generation=False):
    ...
    for i, item in enumerate(items_list):
        ...
        if (i+1) % edit_freq == 0 or (i + 1) == len(items_list):
            ...
            lora_weights, exec_time = cake_batch_kl_return_lora_weights(current_model, tokenizer, accumulated_items, target_modules, hparams, test_generation)
