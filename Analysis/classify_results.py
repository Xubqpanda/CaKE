...
def load_activation_patching(dataset_dir, case_name):
    generations = defaultdict(dict)
    layers = defaultdict(dict)
    for source in ["e1", "r1", "r2", "last"]:
        for target in ["composition", "second-hop", "original", "last"]:
            path = f"{dataset_dir}/activation_patching/{source}_{target}_activation_patching_{case_name}.csv"
            if Path(path).exists():
